...
import questionsData from '../data/subject-b-training.json';
...
const categories = ['Audit Program Design', 'Control Evaluation', 'Finding Development', 'Compliance Assessment', 'Comprehensive Audits'];
...
export default function SubjectBTraining() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('au-b-stats');
      if (saved) setStats(JSON.parse(saved));
    } catch {}
  }, []);

  useEffect(() => {
    localStorage.setItem('au-b-stats', JSON.stringify(stats));
  }, [stats]);
  ...
  function resetStats() {
    if (confirm('Reset all Subject B stats?')) {
      ...
      {q ? (
        <div className="space-y-4">
          <div className="flex justify-between items-center text-xs text-dark-400">
            <span className="bg-audit-900/50 text-audit-300 px-2 py-0.5 rounded">{q.category}</span>
