...
export default function QuizPlayPage({ category, mode, onBack }: QuizPlayPageProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return pool;
  }, [questions, category, mode, wrongIds]);
  ...
  const modeLabel = mode === 'wrong' ? '間違い直し' : (category === 'all' ? '全問' : categoryNames[category] || category);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-dark-400 px-2 py-0.5 bg-dark-800 rounded-full">{modeLabel}</span>
        <span className="text-xs text-dark-400">
          {stats.total > 0 ? `正解率 ${Math.round((stats.correct / stats.total) * 100)}%` : ''}
        </span>
      </div>
        ...
        <div className="space-y-4">
          ...
          {selected !== null && currentIdx === filtered.length - 1 && (
            <div className="text-center py-4">
              <p className="text-audit-400 font-medium mb-3">完了! {filtered.length}問終了しました。</p>
              <button onClick={reset} className="text-sm text-dark-400 hover:text-audit-400 transition-colors underline">
                もう一度やる
              </button>
            </div>
          )}
